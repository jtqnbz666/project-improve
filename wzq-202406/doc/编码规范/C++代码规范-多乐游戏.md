...
```
tip
    我们禁止使用 RTTI(Run Time Type Identification)。
```
...
注意：mutable修饰的变量可以突破const的限制，在被const修饰的函数里面也能被修改。
...
如今 constexpr 就可以定义浮点式的真常量，不用再依赖字面值了；也可以定义用户自定义类型上的常量；甚至也可以定义函数调用所返回的常量。
...
靠 constexpr 特性，方才实现了 C++ 在接口上打造真正常量机制的可能。好好用 constexpr 来定义真常量以及支持常量的函数。避免复杂的函数定义，以使其能够与
constexpr一起使用。 千万别痴心妄想地想靠 constexpr 来强制代码「内联」。
...
Boost代码质量普遍较高, 可移植性好, 填补了 C++ 标准库很多空白, 如更完善的绑定器, 更好的智能指针。
...
```
tip:
    适当用 C++11（前身是 C++0x）的库和语言扩展，在项目用 C++11 特性前三思可移植性。
```
...
在2014年8月之前，C++11 一度是官方标准，被大多 C++ 编译器支持。它标准化很多我们早先就在用的 C++ 扩展，简化了不少操作，大大改善了性能和安全。
