...
std::unordered_map<int, SDemoConfig> CDemoConfig::_InitData() {
    std::unordered_map<int, SDemoConfig> mTempData;
...

    {
        ...
        oConfig.fValueFloat = 198.0;
    
        oConfig.vTestint = {3, 4};
    
        oConfig.vKeywords = {"master", "rare"};
    
        oConfig.mStrToIntMap = {{"wood", 999}, {"iron", 50}};
    
        oConfig.vStrPairArray = {{"boss_ticket", 1}, {"gem", 51213131}};
    
        oConfig.sComplexLogic = "";
...
const std::unordered_map<int, SDemoConfig> CDemoConfig::mData = CDemoConfig::_InitData();

const SDemoConfig* CDemoConfig::Get(int nId) {
    std::unordered_map<int, SDemoConfig>::const_iterator it = mData.find(nId);
    if (it != mData.end()) {
        return &it->second;
    }
    return nullptr;
